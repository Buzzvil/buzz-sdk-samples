...
import com.buzzvil.buzzbenefit.pop.BuzzPopCustomPageInfo
import com.buzzvil.buzzbenefit.pop.BuzzPopNavigator
...
class CustomPopToolbarHolder: DefaultBuzzPopToolbarHolder() {
    override fun getView(activity: Activity): View {
        super.toolbar = BuzzPopToolbarLayout(activity) // PopToolbar에서 제공하는 기본 Template 사용
        ...
        addRightMenuItemView(activity) {
            super.showInquiry(activity) // 문의하기 페이지로 연결합니다.
        }
        addRightMenuItemView(activity) {
            BuzzPopNavigator().launchCustomFragment(
                activity,
                BuzzPopCustomPageInfo(
                    YourCustomFragment(),
                    R.string.your_custom_fragment_title
                )
            )
        }
    ...
    private fun addRightMenuItemView(activity: Activity, clickListener: () -> Unit) {
