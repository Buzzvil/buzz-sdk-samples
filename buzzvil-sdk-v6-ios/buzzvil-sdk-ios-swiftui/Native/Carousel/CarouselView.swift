...
final class CarouselViewController: UIViewController {
  ...
  private lazy var nativeGroup = BuzzNativeGroup(unitId: "YOUR_NATIVE_UNIT_ID")
...
//    collectionView.contentInset = UIEdgeInsets(top: .zero, left: 2 * spacing, bottom: .zero, right: 2 * spacing)
  ...
  private func setupLayout() {
    // eg. auto layout constraints for benefitHubEntryView
    benefitHubEntryView.translatesAutoresizingMaskIntoConstraints = false
    NSLayoutConstraint.activate([
      benefitHubEntryView.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 16),
    ...
    NSLayoutConstraint.activate([
      carouselCollectionView.topAnchor.constraint(equalTo: benefitHubEntryView.bottomAnchor, constant: 8),
  ...
  private func loadAds() {
    ...
    nativeGroup.load(
      ...
      onFailure: { [weak self] error in
        // 광고 할당 실패 시 발생하는 오류 코드에 대한 자세한 내용은 오류 코드가 나타납니다 토픽을 참고하세요.
      }
    )
  }
