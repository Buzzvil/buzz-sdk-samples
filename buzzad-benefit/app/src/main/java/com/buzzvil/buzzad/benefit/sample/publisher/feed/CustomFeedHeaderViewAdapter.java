...
public class CustomFeedHeaderViewAdapter implements FeedHeaderViewAdapter {
    ...
    public View onCreateView(final Context context, final ViewGroup parent) {
        ...
        return inflater.inflate(com.buzzvil.buzzad.benefit.R.layout.bz_view_feed_header, parent, false);
    }

    @Override
    public void onBindView(final View view, final int reward) {
        final View rewardLayout = view.findViewById(com.buzzvil.buzzad.benefit.R.id.rewardLayout);
        if (reward == 0) {
            rewardLayout.setVisibility(View.GONE);
        } else {
            rewardLayout.setVisibility(View.VISIBLE);
            final TextView rewardTextView = view.findViewById(com.buzzvil.buzzad.benefit.R.id.rewardText);
